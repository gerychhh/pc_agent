...
## Архитектура
- **Command Library**: набор базовых команд с execute/verify и короткими описаниями (`core/command_library.yaml`).
- **Command Engine**: матчинг запросов без LLM по ключевым словам/регексу, подстановка параметров и запуск команд.
...
## Безопасность
Подтверждения команд отключены. Вместо этого действует строгий валидатор скриптов: опасные операции блокируются автоматически. Примеры запретов:
