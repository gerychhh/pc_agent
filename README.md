...
### verified и ограничения проверки
Инструменты возвращают поле `verified`. Оно может быть `false`, если без OCR нельзя подтвердить результат (например, при вводе текста или клике по интерфейсу). В таких случаях действие выполнено, но результат не подтвержден — это отражается в `verify_reason`.

## Логи
Все запросы, подтверждения и результаты инструментов пишутся в `logs/session_*.jsonl`.

## Поиск приложений
При первом запуске CLI попросит указать папки, где искать установленные программы (например, `C:\Program Files`). Эти пути сохраняются в `app_search_paths.json`, и агент сначала пытается найти `.exe` в указанных каталогах, а затем переходит к другим способам запуска.
