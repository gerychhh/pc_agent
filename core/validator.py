...
def validate_python(script: str) -> list[str]:
    ...
    # Запрещаем создавать бинарные форматы через open(..., 'w')
    if re.search(r"open\([^\n]*\.(docx|xlsx|pdf|pptx)[^\n]*['\"]w", script, re.IGNORECASE):
        errors.append("Нельзя создавать docx/xlsx/pptx/pdf через open(...,'w').")

    # Если используется Path, нужен импорт
...
def validate_powershell(script: str) -> list[str]:
    ...
    # Блокируем создание/редактирование docx/xlsx/pptx как обычного текста
    if re.search(
        ...
        errors.append(
            ...
            "Используй python-docx/openpyxl/python-pptx или COM Office."
        )
